...
def square_sig(A: f32, f: f32, phi: f32):
    def signal(t):
...
d = square_sig(A=1, f=f_wave, phi=0)

fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(8, 6), sharey=True)

ax1.plot(t_cont, d(t_cont), label="Square wave (f=300Hz)")
ax1.stem(t_disc, d(t_disc), linefmt="r-", label="Samples")
ax1.set_xlabel("Time(s)")
ax1.set_ylabel("Amplitude")
ax1.grid(True)

ax2.plot(t_cont, d(t_cont), label="Square wave (f=300Hz)")
ax2.stem(t_disc, d(t_disc), linefmt="r-", label="Samples")
