import numpy as np
import matplotlib.pyplot as plt

from src.lib import signal


f_wave = 400
f_sample = 1600
duration_s = 1.0

# oversample the continuous plot by a factor of 20 to make it look smoother
t_cont = np.linspace(0, duration_s, int(duration_s * f_sample * 20), endpoint=False)
t_disc = np.arange(0, duration_s, 1 / f_sample)

a = signal.Sin(A=1, f=f_wave, phi=0)

fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(8, 6))

ax1.plot(t_cont, a(t_cont), label=a)
ax1.stem(t_disc, a(t_disc), linefmt="r-", label="Samples")
ax1.set_xlabel("Time(s)")
ax1.set_ylabel("Amplitude")
ax1.legend()
ax1.grid()

ax2.plot(t_cont, a(t_cont), label=a)
ax2.stem(t_disc, a(t_disc), linefmt="r-", label="Samples")
ax2.set_xlim(0, 1 / f_wave)
ax2.set_xlabel("Time(s)")
ax2.set_ylabel("Amplitude")
ax2.legend()
ax2.grid()

plt.tight_layout()
plt.savefig("src/01/02a.pdf", format="pdf")
plt.show()
